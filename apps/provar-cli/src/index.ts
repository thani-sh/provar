...
function renderTraceReport(trace: any) {
  ...
  const totalSandboxDurationMs = trace.totalTimings["sandbox"]?.reduce((a: number, b: number) => a + b, 0) || 0;
  const totalAgentCount = trace.totalTimings["agent"]?.length || 0;
  const totalAgentDurationMs = trace.totalTimings["agent"]?.reduce((a: number, b: number) => a + b, 0) || 0;

  console.log(pc.bold(pc.cyan(`\n🌌 Provar Telemetry - Performance Trace Report`)));
  console.log(pc.dim("-----------------------------------------------------------------------------------------"));
  console.log(`${pc.bold("Compile Target:")} ${pc.cyan(trace.target)}`);
  console.log(`${pc.bold("Total Duration:")} ${pc.yellow(trace.totalDurationMs.toFixed(2) + " ms")}`);
  console.log(`${pc.bold("Agent Client Setup:")} ${pc.yellow(trace.setupDurationMs.toFixed(2) + " ms")}`);
  console.log(`${pc.bold("Total Sandbox Runs:")} ${pc.yellow(totalSandboxCount)} (${totalSandboxDurationMs.toFixed(2)} ms total)`);
  console.log(`${pc.bold("Total Agent Requests:")} ${pc.yellow(totalAgentCount)} (${totalAgentDurationMs.toFixed(2)} ms total)`);
  console.log(pc.dim("-----------------------------------------------------------------------------------------"));
  ...
  const otherDuration = Math.max(0, total - (trace.parseDurationMs + trace.setupDurationMs + totalSandboxDurationMs + totalAgentDurationMs + trace.writeDurationMs));
  const otherPct = ((otherDuration / total) * 100).toFixed(1);

  console.log(`  +-----------------------------------+-----------------------------------+`);
  console.log(`  | ${pc.bold("Phase")}                             | ${pc.bold("Duration (ms)")}                       |`);
  console.log(`  +-----------------------------------+-----------------------------------+`);
  console.log(`  | Parsing & Resolving Paths         | ${trace.parseDurationMs.toFixed(1).padStart(12)} ms (${parsePct.padStart(5)}%)      |`);
  console.log(`  | Agent Client Initialization       | ${trace.setupDurationMs.toFixed(1).padStart(12)} ms (${setupPct.padStart(5)}%)      |`);
  console.log(`  | Playwright Sandbox Executions     | ${totalSandboxDurationMs.toFixed(1).padStart(12)} ms (${sandboxPct.padStart(5)}%)      |`);
  console.log(`  | Agent Action Code Generation      | ${totalAgentDurationMs.toFixed(1).padStart(12)} ms (${agentPct.padStart(5)}%)      |`);
  console.log(`  | File Serialization & Disk Writes  | ${trace.writeDurationMs.toFixed(1).padStart(12)} ms (${writePct.padStart(5)}%)      |`);
  console.log(`  | Other Compiler Overhead           | ${otherDuration.toFixed(1).padStart(12)} ms (${otherPct.padStart(5)}%)      |`);
  console.log(`  +-----------------------------------+-----------------------------------+`);

  console.log(pc.bold("\nTask-by-Task Bottlenecks (Sorted by Duration):"));
  console.log(`  +----------------------+----------+---------------+------------+----------+-----------+`);
  console.log(`  | ${pc.bold("Task ID")}               | ${pc.bold("Status")}   | ${pc.bold("Total Time")}    | ${pc.bold("Sandboxes")}  | ${pc.bold("Agent Req")} | ${pc.bold("Mode")}      |`);
  console.log(`  +----------------------+----------+---------------+------------+----------+-----------+`);

  const sortedTasks = [...(trace.tasks || [])].sort((a, b) => b.durationMs - a.durationMs);
  ...
  for (const task of sortedTasks) {
    ...
    else if (task.status === "HEALED") statusStr = pc.yellow(task.status.padEnd(8));
    else if (task.status === "FAILED") statusStr = pc.red(task.status.padEnd(8));
    ...
    console.log(`  | ${idStr} | ${statusStr} | ${timeStr} | ${sandboxesStr} | ${agentStr} | ${modeStr} |`);
  }
  console.log(`  +----------------------+----------+---------------+------------+----------+-----------+`);
}
