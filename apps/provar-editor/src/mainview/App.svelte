<script lang="ts">
  ...
  } from "./lib/components/AssistantPanel.svelte";
  ...
  async function handleConfigConfirm(newConfig: ProvarConfig) {
    ...
    try {
      ...
      if (res.success) {
        ...
        alert(
          "Failed to save configuration. Please check your workspace permissions.",
        );
      }
    } catch (e) {
  ...
  async function handleAddNode(fromId: string | null, toId: string | null) {
    ...
    const newContent = JSON.parse(
      JSON.stringify(currentFileContent),
    ) as TestFile;
...
>
  ...
  >
    {#if !workspacePath}
      <div
        class="flex flex-col items-center justify-center h-full text-zinc-500 space-y-4"
      >
        <p class="text-xl">Open a workspace to get started</p>
        <div class="flex items-center space-x-2 text-sm text-zinc-600">
          <kbd
            class="px-2 py-1 bg-zinc-800 rounded border border-zinc-700 font-mono"
            >Cmd/Ctrl + O</kbd
          >
