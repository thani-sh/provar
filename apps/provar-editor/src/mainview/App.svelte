<script lang="ts">
	...
	import type { ProvarConfig, TestFile, TestNode } from '../shared/domain';
	...
	async function handleConfigConfirm(newConfig: ProvarConfig) {
		...
		try {
			const res = await electroview.rpc.request.saveConfig({
				config: newConfig
			});
			if (res.success) {
				config = newConfig;
	...
	function handleCreateFile(parentPath: string) {
		modalType = 'file';
		modalTitle = 'New test';
		modalPlaceholder = 'Enter test name...';
	...
	async function handleModalConfirm(name: string) {
		...
		try {
			let dir = modalParentPath.endsWith('.yml')
				? modalParentPath.split('/').slice(0, -1).join('/')
				: modalParentPath;
	...
	async function handleAddNode(fromId: string | null, toId: string | null) {
		...
		await electroview.rpc.request.writeFile({
			path: selectedFile,
			content: newContent
		});
	...
	async function handleAssist(prompt: string) {
		...
		try {
			...
			assistantMessages = assistantMessages.map((msg) =>
				msg.id === assistantMsgId ? { ...msg, content: res.message, status: 'completed' } : msg
			);
...
>
	...
	>
		{#if !workspacePath}
			<div class="flex h-full flex-col items-center justify-center space-y-4 text-zinc-500">
				<p class="text-xl">Open a workspace to get started</p>
				<div class="flex items-center space-x-2 text-sm text-zinc-600">
					<kbd class="rounded border border-zinc-700 bg-zinc-800 px-2 py-1 font-mono"
						>Cmd/Ctrl + O</kbd
					>
		...
		{:else if currentFileContent}
			<Canvas testFile={currentFileContent} bind:selectedNodeId onAddNode={handleAddNode} />
		{:else}
	...
	{:else if configPanelShow}
		<ConfigPanel {config} onSave={handleConfigConfirm} />
	...
	<ConfigModal show={configModalShow} onConfirm={handleConfigConfirm} />
</div>
