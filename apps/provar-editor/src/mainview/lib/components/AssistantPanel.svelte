    ...
    {:else}
      {#each messages as msg}
        <div class="flex gap-3 {msg.role === 'user' ? 'flex-row-reverse' : ''}">
          <div class="mt-1 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-zinc-800 bg-[#21262d]">
            {#if msg.role === 'user'}
          ...
          <div class="flex flex-col gap-1 {msg.role === 'user' ? 'items-end' : ''}">
            <div class="rounded-2xl px-4 py-2 text-sm leading-relaxed {msg.role === 'user' ? 'bg-indigo-600 text-white' : 'bg-zinc-800/50 text-zinc-200'}">
              {#if msg.role === 'assistant' && msg.status === 'pending'}
                <span class="italic text-zinc-400">Thinking...</span>
              {:else}
                {msg.content}
              {/if}
            </div>
  ...
  <div class="p-4 border-t border-zinc-800/50">
    {#if fileName}
      <div class="mb-2 flex items-center px-1">
        <div class="flex items-center gap-1 rounded-full border border-zinc-700/50 bg-[#21262d] py-0.5 pl-1.5 pr-2 text-[10px] text-zinc-400">
    ...
    <div class="relative">
      <textarea
        ...
        class="w-full bg-[#0d1117] border border-zinc-700/50 rounded-lg p-3 text-sm text-zinc-200 focus:outline-none focus:border-indigo-500 min-h-[100px] resize-none {isBusy ? 'opacity-50 cursor-not-allowed' : ''}"
        onkeydown={(e) => {
