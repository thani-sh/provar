...
export class ConnectorShape extends PIXI.Container {
	...
	constructor(
		...
		if (type === 'horizontal') {
			...
			this.line.bezierCurveTo(startX + curveStrength, startY, endX - curveStrength, endY, endX, endY);
			...
			const p1x = startX + curveStrength, p1y = startY;
			const p2x = endX - curveStrength, p2y = endY;
		...
		if (type === 'horizontal') {
			const curveStrength = Math.abs(endX - startX) / 2;
			this.hitAreaLine.bezierCurveTo(startX + curveStrength, startY, endX - curveStrength, endY, endX, endY);
		} else {
		...
		this.on('pointerover', () => {
			...
			this.line.stroke({ color: COLOURS.primary, width: CONNECTOR.lineWidth + 1 });
		});

		this.on('pointerout', () => {
			this.addButton.visible = false;
			this.line.stroke({ color: COLOURS.connector, width: CONNECTOR.lineWidth });
		});
	}
