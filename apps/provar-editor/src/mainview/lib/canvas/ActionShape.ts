	...
	constructor(nodeId: string, node: TestNode, onClick: (id: string) => void) {
		...
		if (node.graph) {
			addIcon((g) => {
				...
				g.moveTo(4, 4);
				g.lineTo(6, 6);
				g.stroke({ color: COLOURS.iconNeutral, width: 1.5, join: 'round', cap: 'round' });
			});
		}

		const codeStatus = getCodeStatus(node);
		const codeColor = codeStatus === 'upToDate' ? COLOURS.codeUpToDate : COLOURS.codeOutdated;
    ...
    // I'll skip pixel diff icon for now unless I add it to the schema.
		...
		if (node.asserts && Object.keys(node.asserts).length > 0) {
			addIcon((g) => {
				...
				g.lineTo(4, 8);
				g.lineTo(9, 2);
				g.stroke({ color: COLOURS.assertGreen, width: 1.5, join: 'round', cap: 'round' });
			}, 0.8);
		}
