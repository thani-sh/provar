...
export class Viewport extends PIXI.Container {
	...
	constructor(private readonly app: PIXI.Application) {
		...
		this.app.canvas.addEventListener('wheel', this.preventNativeWheel, { passive: false });
	...
	private updateHitArea() {
		if (this.hitArea instanceof PIXI.Rectangle) {
			...
			const bottomRight = this.toLocal({ x: this.app.screen.width, y: this.app.screen.height });
	...
	private onWheel(e: PIXI.FederatedWheelEvent): void {
		if (e.metaKey || e.ctrlKey) {
			...
			const newScale = Math.min(Math.max(this.scale.x * factor, this.minScale), this.maxScale);
