...
import { setWorkspaceDir, WORKSPACE_DIR, onWorkspaceChanged, getAbsPath } from "./utils";
...
const runTestPath = async (params: { path: string; pathIndex: number; upToActionId?: string; headless?: boolean }) => {
  const absPath = getAbsPath(params.path);
  console.log("[RPC Server] runTestPath request parameters:", { ...params, path: absPath });
  try {
    ...
    if (needCompile) {
      ...
      if (!compileRes.success) {
        throw new Error("Auto-compilation failed. Please compile manually to check errors.");
      }
    }
    ...
    if (params.pathIndex < 0 || params.pathIndex >= execFile.paths.length) {
      ...
      return { success: false, error: `Invalid path index ${params.pathIndex}` };
    }
    
    const selectedPath = execFile.paths[params.pathIndex];
    if (!selectedPath) {
      return { success: false, error: `Path at index ${params.pathIndex} not found` };
    }
    const runId = Math.random().toString(36).substring(7);
    
    console.log("[RPC Server] Spawning path runner, selected path task count:", selectedPath.tasks.length);
    ...
    (async () => {
      ...
      const relativePath = path.relative(testsDir, absPath).replace(".test.yml", "");
      const pathNameSlug = selectedPath.tasks
        .map(t => t.id.replace(/^action_/, ""))
        .join("-");
      
      for await (const event of runner.events()) {
        console.log(`[RPC Server] Runner Event: ${event.type} for runId: ${runId}`, "taskId" in event ? event.taskId : "");
        if (event.type === "visual-comparison-triggered") {
          try {
            const taskIndex = selectedPath.tasks.findIndex(t => t.id === event.taskId);
            const stepIndexStr = String(taskIndex + 1).padStart(3, "0");
            const actionId = event.taskId.replace(/^action_/, "");
            
            const currentDir = path.join(WORKSPACE_DIR, ".provar", "screenshots", "current", relativePath, pathNameSlug);
            fs.mkdirSync(currentDir, { recursive: true });
            const currentFilePath = path.join(currentDir, `${stepIndexStr}_${actionId}.png`);
            fs.writeFileSync(currentFilePath, Buffer.from(event.screenshotBase64, "base64"));
          } catch (e) {
        ...
        mainWindow.webview.rpc?.send.testRunEvent({
          params: {
            ...
            error: "error" in event ? (event.error?.message || String(event.error)) : undefined,
            screenshotBase64: "screenshotBase64" in event ? event.screenshotBase64 : undefined,
            visualCompare: "visualCompare" in event ? event.visualCompare : undefined,
          }
        });
      }
      ...
      console.log(`[RPC Server] Runner loop ended for runId: ${runId}. Status: ${finalState.status}`);
      mainWindow.webview.rpc?.send.testRunEvent({
        params: {
          runId,
          type: "run-finished",
...
const acceptVisualState = async (params: { testPath: string; pathIndex: number; actionId: string }) => {
  const absPath = getAbsPath(params.testPath);
  console.log("[RPC Server] acceptVisualState request:", { ...params, testPath: absPath });
  try {
    const project = await loadProject(absPath);
    const execFile = await project.readFile(absPath);
    const selectedPath = execFile.paths[params.pathIndex];
    ...
    const taskIndex = selectedPath.tasks.findIndex(t => t.id === params.actionId);
    if (taskIndex === -1) {
      console.error("[RPC Server] acceptVisualState error: action not found in path:", params.actionId);
      return { success: false, error: `Action ${params.actionId} not found in path` };
    }
    const stepIndexStr = String(taskIndex + 1).padStart(3, "0");
    const shortActionId = params.actionId.replace(/^action_/, "");
    
    const screenshotFile = `${stepIndexStr}_${shortActionId}.png`;
    const currentFilePath = path.join(WORKSPACE_DIR, ".provar", "screenshots", "current", relativePath, pathNameSlug, screenshotFile);
    const acceptedFilePath = path.join(WORKSPACE_DIR, ".provar", "screenshots", "accepted", relativePath, pathNameSlug, screenshotFile);
    
    if (!fs.existsSync(currentFilePath)) {
      console.error("[RPC Server] acceptVisualState error: current screenshot does not exist:", currentFilePath);
      return { success: false, error: "Current screenshot does not exist. Run the test first." };
    }
...
const getScreenshots = async (params: { testPath: string; pathIndex: number; actionId: string }) => {
  const absPath = getAbsPath(params.testPath);
  console.log("[RPC Server] getScreenshots request:", { ...params, testPath: absPath });
  try {
    const project = await loadProject(absPath);
    const execFile = await project.readFile(absPath);
    const selectedPath = execFile.paths[params.pathIndex];
    if (!selectedPath) {
      console.warn("[RPC Server] getScreenshots warning: path not found at index:", params.pathIndex);
      return {};
    }
    ...
    const taskIndex = selectedPath.tasks.findIndex(t => t.id === params.actionId);
    if (taskIndex === -1) {
      console.warn("[RPC Server] getScreenshots warning: action not found in path:", params.actionId);
      return {};
    }
    ...
    console.log("[RPC Server] getScreenshots response baseline exists:", !!res.baseline, "current exists:", !!res.current);
    return res;
  } catch (err) {
