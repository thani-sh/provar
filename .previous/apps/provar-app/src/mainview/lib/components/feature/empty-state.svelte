<script lang="ts">
  ...
  async function pickExisting() {
    const res = await ProvarAPI.selectProject();
    if (!res.success) {
      onError("Could not open a folder. Please try again.");
    }
  }
  ...
  async function createSample() {
    ...
    busy = true;
    try {
      const res = await ProvarAPI.createSampleProject();
      if (!res.success) {
        onError(res.error ?? "Could not create the sample project.");
        return;
      }
      if (res.path) onOpen(res.path);
    } finally {
      busy = false;
    }
  }
...
>
  <div class="w-full max-w-2xl">
    ...
    <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
      <!-- Card 1: Create sample (the recommended path) -->
      <button
