...
2. It bypassed the centralized app settings (stored in `~/.provar/settings.json`), preventing users from configuring credentials in the UI.
3. The protocol complexity around FS proxying and permissions was unnecessary, as all operations can be performed in the Electrobun main process.
...
4. The `assistEditor` RPC handler in `apps/provar-app` will load settings and instantiate the agent client/session to stream tokens directly back to the webview via the `assistantChunk` RPC message.
