...
The Provar desktop application (`@apps/provar-app`) currently implements filesystem and configuration capabilities (such as reading, creating, updating, and deleting test YAML files/folders, and reading/writing configuration files) within its own workspace boundaries. To enable AI agents to perform these actions autonomously via tool calls, these capabilities must be decoupled from the UI/desktop runtime, structured under a consistent command pattern, and placed in a shared library.
...
We will migrate the raw serialization schemas from the app workspace to `@libs/domain/zod` and reuse existing schema schemas (e.g. `schemaForTaskConfig`) where appropriate. 
...
- **Path Security**: All commands will resolve absolute paths via a safe path helper (`getAbsPath`) using the `workspaceDir` parameter, protecting against directory traversal.
